...
int is_parity(Emulator* emu) { return (emu->eflags & PARITY_FLAG) != 0; }

int is_interrupt(Emulator* emu) { return (emu->eflags & INTERRUPT_FLAG) != 0; }

void update_eflags_sub8(Emulator* emu, uint8_t v1, uint8_t v2,
                        uint16_t result) {
  /* 各値の符号を取得 */
  int sign1 = v1 >> 7;
  int sign2 = v2 >> 7;
...
void update_eflags_add8(Emulator* emu, uint8_t v1, uint8_t v2,
                        uint16_t result) {
  // int sign1 = v1 >> 31;
  // int sign2 = v2 >> 31;
  int signr = (result >> 7) & 1;
...
void update_eflags_sar(Emulator* emu, uint32_t v1, uint8_t v2,
                       uint32_t result) {
  // int sign1 = v1 >> 31;
  // int sign2 = v2 >> 31;
  int signr = (result >> 31) & 1;
...
void update_eflags_shr(Emulator* emu, uint32_t v1, uint8_t v2,
                       uint32_t result) {
  int sign1 = (v1 >> 31) & 1;
  // int sign2 = v2 >> 31;
  int signr = (result >> 31) & 1;
